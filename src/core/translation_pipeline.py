...
'''
            ...
            # Also create date translations (weekdays, months)
            self._create_date_translations(game_dir)
            
        except Exception as e:
            self.log_message.emit("warning", self.config.get_ui_text("pipeline_lang_init_failed").format(error=e))
    
    def _create_date_translations(self, game_dir: str):
        """Create common.rpy with date/time translations for the target language."""
        try:
            from src.core.output_formatter import DATE_TRANSLATIONS, ENGLISH_WEEKDAYS, ENGLISH_WEEKDAYS_SHORT, ENGLISH_MONTHS, ENGLISH_MONTHS_SHORT
            
            if self.target_language not in DATE_TRANSLATIONS:
                self.log_message.emit("info", f"No date translations available for {self.target_language}")
                return
            ...
            if os.path.exists(common_file):
                self.log_message.emit("info", f"Date translations file already exists: {common_file}")
                return
            
            trans = DATE_TRANSLATIONS[self.target_language]
            
            lines = [
                f"# {self.target_language.title()} translations for Ren'Py common strings",
            ...
            # Weekdays
            for eng, loc in zip(ENGLISH_WEEKDAYS, trans["weekdays"]):
            ...
            self.log_message.emit("info", f"Created date translations: {common_file}")
            
        except Exception as e:
            self.log_message.emit("warning", f"Could not create date translations: {e}")
